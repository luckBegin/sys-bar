...
class DateDeal {
	...
	static transtoTimeStamp = function (dataStr: string | number | Date): any {
	...
	compare(start: string | number, end: string | number = Date.now()): boolean {
	...
	timeDiff(startTime: string | number, endTime: string | number): { d: number, h: number, m: number, s: number } {
		...
		if (endTime < startTime) {
			endDate = this.format(Date.now() + this.timeStampAddDay(1), 'y-m-d');
		} else {
	...
	toCompleteDate(startTime: string, dataStr: string): string {
		const now = startTime;
		const timeStamp = this.getNow();
		let date;
		if (dataStr > now) {
			date = this.format(timeStamp, 'y-m-d')
		} else {
			date = this.format(this.timeStampAddDay(1) + <number>timeStamp, 'y-m-d');
		}
		
		return date + ' ' + RegUtils.delimiter(dataStr, 2, ':') + ':00';
	};
}
